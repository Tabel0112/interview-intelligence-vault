...
>
> **Critical accuracy rule.** The deterministic Ask AI claim text and the `token-hash-v1` embedding vectors remain **fallback / test / local-only modes**, not the finished MVP. Real external LLM synthesis and a real external embedding provider now **exist and are wired**, but are used **only when explicitly configured with a valid key**; otherwise the app falls back to deterministic local mode. Never describe token-hash vectors as semantic.
>
...
## 9. Ask AI — deterministic fallback + live grounded LLM synthesis
**[CURRENT]** The 9-step pipeline (`src/ask-ai/pipeline.ts`) is implemented and wired live (`Plugin → ObsidianAppApi → createSqliteFrontendApi → askAI`, via the per-ask `getSynthesis` getter). When no LLM is configured, `generateClaimsFromEvidence` produces **deterministic templated claim text** from the top cited quote. When an external LLM is configured, a real grounded LLM synthesizes claims (§10) over the **selected evidence only**, passed through the grounding gate (§15). Pipeline, citations, conflict handling, refusal, and persistence work in both modes; persisted synthesis metadata is **runtime-accurate** (records the actual path — `llm` vs deterministic — and `usedFallback`). **[FUTURE]** streaming, multi-turn, tool-use, caching.

## 10. External LLM grounded synthesis
**[CURRENT]** A concrete `AskAILanguageModel` (`src/ask-ai/llmSynthesis.ts`, `createLlmAskAILanguageModel`) backed by the LLM provider abstraction (§22): receives the **query + selected evidence only**, returns structured claims citing only selected pointers, wired via DI, output treated as untrusted and passed through the validation gate (§15). The LLM provider stack lives in `src/llm/` (`LlmProvider`, local-deterministic + mock providers, `ExternalLlmProvider` OpenAI-compatible chat/completions with injectable transport, error hierarchy, timeout/cancellation, redaction, `#private` key fields). Settings→LLM resolution is `src/obsidian/llmSettings.ts` (`resolveLlmProviderFromSettings`, `askAiSynthesisFromSettings`). **Inherited guarantees:** claims filtered to selected pointers; uncited claims dropped; `renderAnswer` throws on a claim without a citation; `persistAskAIResponse` throws on broken/mismatched/unselected pointers; the 9-step order is fixed; any LLM failure/malformed/timeout/all-discarded → deterministic fallback. **[MVP]** evidence token-budget/truncation for large contexts; deeper grounding (§15).

## 11. LLM-backed memory extraction
**[CURRENT]** Both modes exist and run live. Deterministic: `DeterministicRuleExtractor` (regex). LLM: `src/memory/extraction/llmExtractor.ts` (`createLlmMemoryExtractor`) — grounded (quote-anchored, span-membership-checked) with deterministic per-window fallback. **Extraction runs automatically after import** (`uploadTranscript` uses the settings-resolved extractor; `memoryExtractorFromSettings`), idempotent. **LLM-extracted memory is capped to `needs_review`** (never auto-`active`); insert-then-promote and evidence-required promotion are preserved; prompt-version metadata is recorded per extractor. **[FUTURE]** cross-span synthesis, entity resolution, summary objects.

## 12. API key / settings requirements
**[CURRENT]** Settings expose provider selection, per-capability model selection, and key handling; keys resolve to concrete LLM/embedding providers via DI; unset ⇒ deterministic local mode. **Contract:** **Obsidian has no secret store** — plugin `data.json` is plaintext and may sync — so key storage is a deliberate decision; keys are held in `#private` fields, never logged, persisted into vault data/SQLite, or surfaced in errors/health/Markdown (asserted by the smoke-test whole-table secret sweep); vault content is never sent externally without explicit configuration; no key ⇒ local deterministic mode. **[FUTURE]** OS-keychain / outside-vault key storage.
...
**[CURRENT]** the default mode when unconfigured: token-hash/noop embeddings, deterministic claim text, rule extractor — fully deterministic, no network, CI-safe. **Contract (binding on all states):** local mode is the default when unconfigured; **tests always run here** (no external service, no network — external providers are exercised only via injected mock transports); CLAUDE.md determinism rules apply (no unmocked `Date.now`/`Math.random` in ranking/scoring/migration/pipeline); real providers are injected and mockable; never a network call inside a SQLite transaction.
