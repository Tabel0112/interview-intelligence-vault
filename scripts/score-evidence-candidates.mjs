...
import { scoreAllEvidenceCandidates } from "../src/evidenceScoringFiltering.mjs";
...
if (transcriptIndex !== -1 && !transcriptId) {
  ...
  try {
    const results = await scoreAllEvidenceCandidates({
    ...
    console.log(`Evidence cards selected: ${results.selectedCount}`);
